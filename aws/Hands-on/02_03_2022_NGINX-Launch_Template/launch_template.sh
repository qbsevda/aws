...
Purpose of the this hands-on training is understanding of 
how to create Launch Template on AWS Console with `user data` and how to version Launch Templates.
...
19. Select `MyTemplate` ---> `Actions` ---> `Launch Instance from Template`
...
24. Select template named `MyTemplate` ---> `Actions` ---> `Modify template (Create New Version)`
...
29. Go to `Launch Template` Menu and click on `MyTemplate`.
...
40. Go to `Launch Template` Menu and click on `MyTemplate`.
